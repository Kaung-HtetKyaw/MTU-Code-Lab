<template>
  <router-link
    class="article-card relative flex flex-col rounded-lg z-10 cursor-pointer"
  ...
  >
    ...
    >
      <div
        v-if="featured"
        class="absolute inset-x-0 bottom-0 p-6 pt-20 text-white"
        style="background-image: linear-gradient(to top, rgba(74, 85, 104,.4) 40%, rgba(0,0,0,0) 100%)"
      >
        <div
          class="uppercase tracking-wider text-xs font-semibold mb-2"
          v-text="article.frontmatter.tags[0]"
        ></div>
        <div class="font-sans text-2xl border-0 leading-tight font-semibold" v-text="article.title"></div>
      </div>
    </figure>
    <main class="flex flex-1 flex-col bg-white rounded-b-lg p-6" v-if="! featured">
      ...
      <section class="font-sans text-gray-600" v-text="article.frontmatter.description"></section>
    </main>
